...
`Max API` — отдельная расширенная нода для операций актуального Bot API, которые не помещаются в компактный интерфейс обратно совместимой ноды `Max`.

- `Bot`: получение данных бота и полная замена списка команд
- `Message`: расширенная отправка, получение одного или нескольких сообщений, редактирование, сведения о видео и ответ на callback
- `Chat`: изменение данных, действия бота, закрепление сообщений и управление членством бота
- `Chat Member`: получение, добавление и удаление участников
- `Chat Administrator`: получение администраторов, назначение прав и снятие роли
- `Comment`: создание, получение, редактирование и удаление комментариев к постам каналов
- `Subscription`: получение, создание и удаление webhook-подписок

Получение отдельного чата, удаление сообщения и загрузка вложений уже доступны в обратно совместимой ноде `Max` и намеренно не дублируются.

Нода сохраняет `int64`-идентификаторы строками и проверяет их signed-int64 диапазон, автоматически переводит старый официальный API host на `platform-api2.max.ru`, нормализует IDN-домены webhook в Punycode и повторяет отправку при временной обработке медиа. При отказе MAX принять Markdown сообщение один раз повторяется как читаемый plain text.
...
### Расширенные операции MAX Bot API

1. Добавьте ноду `Max API`
2. Выберите ресурс, например `Comment` или `Chat Administrator`
3. Выберите операцию — интерфейс покажет только относящиеся к ней поля
4. Передавайте ID из `Max Trigger` как строки, чтобы не потерять точность `int64`
