...
function buildMessageBody(
	context: IExecuteFunctions,
	itemIndex: number,
	options: { includeNotify: boolean; allowLink: boolean },
): IDataObject {
	...
	if (clearAttachments) {
		...
		body['attachments'] = [...(attachments ?? []), ...(keyboard ? [keyboard] : [])];
	}
...
async function executeMessageOperation(
	...
	if (operation === 'send') {
		...
		return await request(
			...
			buildMessageBody(context, itemIndex, { includeNotify: true, allowLink: true }),
		);
	}
	if (operation === 'update') {
		const messageId = requireString(
