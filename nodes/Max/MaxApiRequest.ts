	...
	while (true) {
		try {
			...
			if (typeof response === 'string') {
				return response.trim().length > 0 ? parseMaxJsonLosslessly(response) : response;
			}
			if (Buffer.isBuffer(response)) {
				const text = response.toString('utf8');
				return text.trim().length > 0 ? parseMaxJsonLosslessly(text) : text;
			}
			return response;
		} catch (error) {
...
function assertSafeNumericIdInput(value: unknown, displayName: string): void {
	if (typeof value === 'number' && Number.isInteger(value) && !Number.isSafeInteger(value)) {
		throw new ApplicationError(
			`${displayName} was provided as an unsafe JavaScript number; pass it as a string to preserve the exact int64 value`,
		);
	}
}
...
export function requireInt64(value: unknown, displayName: string): string {
	assertSafeNumericIdInput(value, displayName);
...
export function parseIdList(value: unknown, displayName: string, maximum = 100): string[] {
	...
	for (const [index, inputValue] of inputValues.entries()) {
		assertSafeNumericIdInput(
			inputValue,
			Array.isArray(value) ? `${displayName} item ${index + 1}` : displayName,
		);
	}
